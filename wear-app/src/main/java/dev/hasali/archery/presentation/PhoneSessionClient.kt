...
data class WearScore(val id: Int, val label: String, val color: Color) {
...
sealed interface WearSessionObservation {
    ...
    data class Active(val state: WearSessionState) : WearSessionObservation
}

class PhoneSessionClient(private val context: Context) {

    fun observeSession(): Flow<WearSessionObservation> = callbackFlow {
        val dataClient = Wearable.getDataClient(context)
        val listener = DataClient.OnDataChangedListener { events ->
            for (event in events) {
                if (event.dataItem.uri.path == "/active-session") {
                    trySend(
                        if (event.type == DataEvent.TYPE_DELETED) WearSessionObservation.Inactive
                        else WearSessionObservation.Active(parseSessionState(event.dataItem))
                    )
                }
            }
        }
        ...
        dataClient.getDataItems("wear://*/active-session".toUri())
            .addOnSuccessListener { items ->
                trySend(
                    if (items.count > 0) WearSessionObservation.Active(parseSessionState(items[0]))
                    else WearSessionObservation.Inactive
                )
                items.release()
            }
    ...
    fun addScore(sessionId: Int, scoreId: Int) {
        sendMessage("/score/add", ByteBuffer.allocate(8).putInt(sessionId).putInt(scoreId).array())
    }
    ...
    private fun parseSessionState(dataItem: DataItem): WearSessionState {
        ...
        val endScores = endLabels.zip(endColors)
